...
class ProfileIn(BaseModel):
    """An investigator is a person in the registry, so the fields that produce their
    الرقم المرجعي are mandatory.

    `MIL-<BRANCH>-<serial>` needs both halves: a serial is unique only within its force, so a
    branch that is missing - or the OTHER catch-all, which is not a namespace - would let two
    real people collapse into one identity and pool their voice prints.
    """

    full_name: str = Field(min_length=1, max_length=200)
    rank: str | None = Field(default=None, max_length=100)
    military_id: str = Field(min_length=1, max_length=64)
    security_branch: SecurityBranch

    @field_validator("security_branch")
    @classmethod
    def _branch_must_be_a_namespace(cls, v: SecurityBranch) -> SecurityBranch:
        if v == SecurityBranch.OTHER:
            raise ValueError("security_branch_other_is_not_a_namespace")
        return v
