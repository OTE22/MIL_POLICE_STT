...
def test_speaker_rename(client, investigator, viewer, admin_token):
    ...
    res = client.patch(f"/api/investigations/{s['id']}/speakers/{speakers['SPEAKER_01']['id']}", json={"display_name": "أحمد محمد", "speaker_role": "SUBJECT", "reference_number": "REF-1"}, headers=auth(investigator["token"]))
