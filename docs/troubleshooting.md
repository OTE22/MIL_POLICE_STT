...
| **فحص البصمات الصوتية** says **تحتاج مراجعة** | Separate computed components or incompatible model groups; this does not prove different people | Use **استماع ومقارنة**. Record a review or explicitly deactivate a wrong sample with a reason. If selected samples are verified as this person, use **تأكيد وحدة الهوية**, within or across groups. See [the review guide](voice-review-panel.md). |
| The automatic result still needs review after **تأكيد وحدة الهوية** | Human confirmation does not rewrite cosine scores or numerical components | Read the separate **نفس الشخص — مؤكّد يدوياً** set and its covered samples. New samples are not automatically included. |
| **تغيّرت العينات — تحتاج إعادة مراجعة** / `voice_review_stale` | A selected sample changed, was deactivated/deleted, or no longer belongs to this person | Refresh the check, listen/review as needed, then confirm the current selection. Even a new per-print review updates the reviewed timestamp. |
| `voice_confirmation_exists` | The same selected set already has an active confirmation | Refresh to inspect it; use **إعادة فتح المراجعة** with a reason if it needs to be withdrawn. |
| The confirmation button is disabled | Fewer than two samples, more than 100 samples, no reason, or a stale result | Select 2–100 samples, enter the reason, and refresh stale results. **مقارنة العينتين المحددتين** needs exactly two selected samples. |
| Source playback is unavailable | Source recording/provenance is missing, audio is unavailable, or the investigation is inaccessible | Check source-session access and `transcripts.read`. Legacy samples without a known source recording cannot substitute a different recording. |
| Many `SPEAKER_*` observations belong to one name | Each recording has its own observations; labels are not person identities | Confirmed observations sharing a canonical person appear in one expandable card. Same-name people and unidentified observations remain separate; inspect recording filenames and **تفاصيل المصدر**. |
