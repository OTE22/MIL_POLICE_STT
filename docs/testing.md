...
**Who this is for:** developers and anyone verifying a release.
...
| محضر تحقيق — schema & attribution | `central/backend/tests/test_report_schema.py`, `test_report_attribution.py` | the four report tables and their constraints (one draft per session, unique report versions, a template version cited by an issued report cannot be deleted, an issued report outlives its draft, no hard FK from a report into evidence); and the attribution join: two recordings both diarized SPEAKER_00 never share a person, the same human across recordings resolves to one identity, legacy rows stay flagged | `docker compose run --rm --no-deps -e SKIP_MIGRATIONS=true -v "$PWD/central/backend:/app" --entrypoint "" backend sh -c "cd /app && python -m pytest tests/test_report_schema.py tests/test_report_attribution.py -q"` |
| محضر تحقيق — context, draft & API | `test_report_context.py`, `test_report_draft.py`, `test_report_api.py` | newest transcript **per recording**, pinning (transcript id + SHA-256 of its segments), staleness detection, turn merging of fragmented segments, س/ج pairing (unanswered questions kept, two people answering one question attributed separately, nothing dropped); edit / merge / split / exclude / restore persistence; and the composer's endpoints with their permission and session-access gates. **`_evidence_fingerprint` hashes every transcript, segment and speaker row before and after each report operation** — the mechanical proof that report editing never touches evidence | same command, `tests/test_report_context.py tests/test_report_draft.py tests/test_report_api.py` |
| محضر تحقيق — renderer & templates | `test_report_renderer.py`, `test_report_templates_api.py` | template validation refuses a renamed zip, empty file, corrupt archive, macros, external relationships, zip bombs, broken Jinja, unknown placeholders, a missing `qa_blocks` loop and sandbox-escape attempts; field codes are warned about (checked **inside** the archive — the XML is deflated); rendering at 1 and at **300** questions; validation gates **activation**, not upload | same command, `tests/test_report_renderer.py tests/test_report_templates_api.py` |
| محضر تحقيق — issuing | `test_report_finalize.py` | the gates (missing fields, unnamed speaker, no content, dev template in production, finalize permission), the frozen snapshot (a later transcript edit leaves an issued report byte-identical), versioning and reopen-for-correction, hashes and verification (a flipped byte → غير مطابق, a missing file → الملف مفقود), cross-session access, and that a failed insert leaves no orphan document | same command, `tests/test_report_finalize.py` |
| الصياغة بالفصحى (optional AI) | `test_llm_formalization.py`, `test_report_fusha.py`, `test_llm_hardware.py` | production can never construct a cloud provider; a missing key degrades to "unavailable with a reason"; the request contract (strict system prompt, low temperature, no reasoning trace); the key never reaches a log or provenance; §74 faithfulness cases; suggest → اعتماد/تعديل/رفض with **no path from model output to printed text without a person**; and the full hardware matrix (no GPU, 8/16/24 GB VRAM, CUDA missing, model missing, runtime down) — all mocked, so they run on any machine | same command, `tests/test_llm_formalization.py tests/test_report_fusha.py tests/test_llm_hardware.py` |
| فحص البصمات الصوتية (advisory coherence check) | `central/backend/tests/test_biometric_check.py` | 404/401, no active prints, single print, coherent pair, incoherent pair → 2 components + REVIEW_REQUIRED, the split-identity case (A↔B 0.84, C↔D 0.86, cross ≈ 0 — every print has a coherent peer yet 2 components are reported), near-duplicate flagged without forcing review, inactive prints excluded, incompatible models grouped separately and never compared, **the check modifies nothing**, no vectors in the response, live threshold changes via `/api/admin/config` (raising عتبة اقتراح الهوية splits a pair; lowering عتبة البصمة شبه المكررة flags one), cross-field rule `near_duplicate_must_exceed_match_threshold` | `docker compose run --rm --no-deps -e SKIP_MIGRATIONS=true -v "$PWD/central/backend:/app" --entrypoint "" backend sh -c "cd /app && python -m pytest tests/test_biometric_check.py -q"` |
...
| محضر تحقيق (2026-08-29): full backend suite | **431 passed, 0 failed**; `tsc --noEmit` clean. Live E2E on the real session: draft built 19 Q&A blocks from 11 transcripts, header fields saved, unresolved speakers acknowledged, report issued, downloaded as a valid .docx containing the Arabic and no unrendered placeholders, verification **سليم**; a flipped byte gave **غير مطابق** |
| محضر تحقيق on the organisation's own template (2026-08-29) | **PASS** — uploaded through the admin page as v3, validated, activated (`production_ready: true`), and report v2 issued on it: 41,617 bytes, 19 Q&A blocks through the `{%tr%}` loop, letterhead (الجمهورية اللبنانية / وزارة الدفاع الوطني / الشرطة العسكرية), محضر ضبط heading, 8 tables and `footer1.xml` all preserved, verification **سليم**. Word visual acceptance is still pending |
| الصياغة بالفصحى against the live model (2026-08-29) | **PASS** — `nvidia/nemotron-3-ultra-550b-a55b`, 1–5 s per block through the service. Faithfulness: `ما بعرف مين أخد السيارة، بس شفت أحمد حدها` → `لا أعلم من أخذ السيارة، إلا أنني رأيت أحمد بجانبها` (negation kept, nothing invented); `شفت حسن، مش حسين` → `رأيت حسناً، لا حسيناً`. A provider 503 mid-run was handled gracefully and the report still issued |
| Local (production-path) runtime detection (2026-08-29) | **PASS** — the backend container reaches a host Ollama through `host.docker.internal` and lists its provisioned models; with `CENTRAL_ENVIRONMENT=production` the cloud provider raises `nvidia_nim_is_development_only` and resolution stays on the local ladder |
| فحص البصمات الصوتية (2026-08-28): pytest + full suite | **14 passed** (`test_biometric_check.py`); **full central suite 257 passed, 0 failed**; `tsc --noEmit` clean |
| فحص البصمات الصوتية live, on the real registry (2026-08-28) | **PASS** — علي عباس (6 prints): `REVIEW_REQUIRED`, **3 مجموعات** exactly matching the hand-measured cosines (a 4-print coherent core at 0.6663–0.7325 + two isolated prints, max 0.6378 / 0.5881); وليد الايوبي (2 prints): `COHERENT` at 0.7556; raising عتبة اقتراح الهوية to 0.70 via إعدادات النظام re-ran as **5 مجموعات**, restoring 0.65 gave 3 again; the invalid threshold pair refused 422; before/after diff of every print proved **nothing modified**; no vectors in responses or logs; browser check (Playwright): button, both dialogs, Arabic statuses, screenshots |
